'''This takes a base MineCraft level and adds coral.
...
# How large (generally) do you want the coral to be?
# Larger numbers result in larger coral.
# LOWERITERATIONS is the minimum size, so if you want coral in
# shallow water, set this small (0 is good)
LOWERITERATIONS = 0
UPPERITERATIONS = 50

# How close to the surface of the water do you want the coral to come?
# Positive numbers will make the coral stick out above the water
# negative numbers will lower it below the surface
# SURFACEHEIGHT = 0 will allow the coral to come just to the surface.
SURFACEHEIGHT = 0

# What material do you want the coral made of?
CORALMAT = 36
...
class Coral(object):
    ...
    offsetlist = [[0,1,-1],[-1,1,0],[0,1,1],[1,1,0]]
    
    def expand(self):
        chunk_number = len(self.chunks)
        for posidx in range(chunk_number):
            position = self.chunks[posidx]
            generation = self.chunkgeneration[posidx]
            if not (generation == self.currentgeneration):
                continue
            
            for throwaway in range(3):
                if random() > 0.7:
                    continue
                newpos = position[:]
                newpos[0] += choice([-1,0,1])
                newpos[1] += choice([0,1,1])
                newpos[2] += choice([-1,0,1])
                if newpos not in self.chunks:
                        self.chunks += [newpos]
                        self.chunkgeneration += [generation+1]
    ...
    def place(self, blocklist, mclevel): 
        for idx in range(len(self.chunks)):
            ...
            y = pos[1]
            z = pos[2]
            assign_value(x,y,z,CORALMAT,blocklist)
    
    def __init__(self, position = [65,50,71]):
        self.origin = position
        self.chunks = [ [position[0],position[1], position[2]] ]
        self.chunkgeneration = [0]
...
def main():
    ...
    print("Making the coral")
    for throwaway in range(CORALNUM):
        # Get a random position
        pos = rnd_surface_point(9,the_map)
        depth = dist_to_mat(pos,[0,-1,0],9,the_map,invert = True)
        if SURFACEHEIGHT > 0:
            effectivedepth = depth
        else:
            effectivedepth = depth + SURFACEHEIGHT
        if effectivedepth < LOWERITERATIONS + 1:
            continue
        pos[1] += 1 - depth
        # Genearte the object
        thiscoral = Coral(pos)
        ...
        if thisiter > depth - 1 + SURFACEHEIGHT:
            thisiter = depth - 1 + SURFACEHEIGHT
        for iteration in range(thisiter):
            thiscoral.expand()
        # Place the hut
        thiscoral.place(blocklist, the_map)
