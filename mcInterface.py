...
class SaveFile(object):
    ...
    def block_to_idx(self, x, y, z):
        '''Convert absolute block coords to an intra-section index.'''
        if y > 255 or y < 0: raise IndexError
        idx = (y%16)*16*16 + (z%16)*16 + (x%16)
        return idx
    ...
    def new_section(self, secidx):
        '''generates a section that is all air'''
        #generate a new section
        newsec = NbtTag10(NbtNew())
        newsec.named = False
        genaraynamess = {'Blocks':4096,'Data':2048,'BlockLight':2048,'SkyLight':2048}
        newY = NbtTag1(NbtNew())
        newY.name = 'Y'
        newY.payload = secidx
        newsec.payload['Y'] = newY
        for newname in genaraynamess:
            newtag = NbtTag7(NbtNew())
            newtag.name = newname
            newtag.payload = [0]*genaraynamess[newname]
            newtag.payload_length = genaraynamess[newname]
            newsec.payload[newname] = newtag
        return newsec
